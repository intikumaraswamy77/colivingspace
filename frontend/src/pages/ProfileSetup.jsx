...
const ProfileSetup = ({ embedded = false }) => {
  ...
  React.useEffect(() => {
    if (userInfo?.profile) {
      setProfile(prev => ({
        ...prev,
        ...userInfo.profile,
        hobbies: userInfo.profile.hobbies?.join(', ') || '',
        languages: userInfo.profile.languages?.join(', ') || ''
      }));
    }
  }, []);
  ...
  return (
    <div className={embedded ? "w-full" : "min-h-screen bg-slate-900 py-12 relative overflow-hidden"}>
      {/* Background Blobs for depth (only if not embedded) */}
      {!embedded && (
      ...
      <div className={embedded ? "w-full" : "max-w-4xl mx-auto px-4 relative z-10"}>
        {!embedded && (
          <div className="text-center mb-10">
            ...
            <p className="text-lg text-slate-400 mt-2">Help our system find you the perfect roommates by answering a few quick questions.</p>
          </div>
        )}

        <form onSubmit={handleSubmit} className={`bg-slate-800/50 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 ${embedded ? 'p-6' : 'p-8 md:p-12'}`}>
          {embedded && (
            <div className="mb-8 border-b border-white/10 pb-4">
              <h2 className="text-3xl font-extrabold text-white tracking-tight">Your Profile</h2>
          ...
          {success && <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl mb-8">{success}</div>}
          ...
          <div className="space-y-12">
            ...
            <section>
              ...
              <div className="mb-6">
                ...
                <textarea id="bio" name="bio" value={profile.bio} onChange={handleChange} rows="3" required placeholder="Tell potential roommates a bit about yourself..." className="w-full bg-slate-900/50 border border-white/10 text-white placeholder-slate-500 rounded-xl p-4 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none resize-none transition-all"></textarea>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="age" className="block text-sm font-bold text-slate-300 mb-2">Age</label>
                  <input id="age" type="number" name="age" required min="16" max="120" value={profile.age} onChange={handleChange} className="w-full bg-slate-900/50 border border-white/10 text-white rounded-xl p-4 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input id="budget" type="number" name="budget" required min="0" value={profile.budget} onChange={handleChange} placeholder="e.g. 15000" className="w-full bg-slate-900/50 border border-white/10 text-white placeholder-slate-500 rounded-xl p-4 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all" />
                </div>
              </div>
            </section>

            {/* Lifestyle - Only show for tenants */}
            {userInfo?.role !== 'owner' && (
              <section>
            ...
            {userInfo?.role !== 'owner' && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input id="hobbies" type="text" name="hobbies" value={profile.hobbies} required onChange={handleChange} placeholder="Reading, Gaming, Gym" className="w-full bg-slate-900/50 border border-white/10 text-white placeholder-slate-500 rounded-xl p-4 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all" />
                </div>
                <div>
                  <label htmlFor="languages" className="block text-sm font-bold text-slate-300 mb-2">Languages (comma separated)</label>
                  <input id="languages" type="text" name="languages" value={profile.languages} required onChange={handleChange} placeholder="English, Hindi" className="w-full bg-slate-900/50 border border-white/10 text-white placeholder-slate-500 rounded-xl p-4 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-all" />
                </div>
              </div>
            </section>
            )}
          ...
          <div className="mt-12 pt-8 border-t border-white/10 flex justify-end">
            <button type="submit" disabled={loading} className="bg-indigo-600 text-white font-bold text-lg px-12 py-4 rounded-xl hover:bg-indigo-500 transition-all shadow-[0_0_15px_rgba(79,70,229,0.3)] hover:shadow-[0_0_25px_rgba(79,70,229,0.5)] disabled:opacity-50">
              {loading ? 'Saving Profile...' : embedded ? 'Save Profile' : 'Save & Generate Compatibility Profile'}
            </button>
          </div>
        </form>
      </div>
