...
const AddProperty = ({ onPropertyAdded }) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const imagesArray = formData.images ? formData.images.split(',').map(item => item.trim()).filter(url => url) : [];
      ...
      setError('');
      setFormData({
        title: '', description: '', location: '', roomType: 'Shared Room', rent: '', deposit: '', capacity: '', amenities: '', bedrooms: '', bathrooms: '', floor: '', furnishing: 'Unfurnished', genderPreference: 'Any', availableDates: '', images: ''
      });
  ...
  return (
    <div className="bg-slate-800/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white/10 mt-8">
      ...
      <form onSubmit={submitHandler} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <input id="capacity" type="number" name="capacity" required value={formData.capacity} onChange={handleChange} className="block w-full rounded-xl bg-slate-900/50 border-white/10 text-white placeholder-slate-500 border p-3 pl-10 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all outline-none" placeholder="2" />
            </div>
          </div>
          <div>
            ...
            <div className="relative mt-1">
              ...
              <input id="rent" type="number" name="rent" required value={formData.rent} onChange={handleChange} className="block w-full rounded-xl bg-slate-900/50 border-white/10 text-white placeholder-slate-500 border p-3 pl-10 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all outline-none" placeholder="10000" />
            </div>
          </div>
          <div>
            ...
            <div className="relative mt-1">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><DollarSign className="h-5 w-5 text-slate-500" /></div>
              <input id="deposit" type="number" name="deposit" required value={formData.deposit} onChange={handleChange} className="block w-full rounded-xl bg-slate-900/50 border-white/10 text-white placeholder-slate-500 border p-3 pl-10 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all outline-none" placeholder="20000" />
            </div>
          </div>
        </div>
        ...
        <div>
          <label htmlFor="images" className="block text-sm font-medium text-slate-300">Image URLs (comma separated)</label>
          <input id="images" type="text" name="images" value={formData.images} onChange={handleChange} className="mt-1 block w-full rounded-xl bg-slate-900/50 border-white/10 text-white placeholder-slate-500 border p-3 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all outline-none" placeholder="https://image1.jpg, https://image2.jpg" />
          <p className="text-xs text-slate-500 mt-1">Paste public image URLs to showcase your property.</p>
        </div>
